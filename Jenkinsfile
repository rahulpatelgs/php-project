pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t rahulpatel123/2febimg:v1 .'
                    sh 'docker images'
                }
            }
        }
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub_credentials', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push rahulpatel123/2febimg:v1'
                }
            }
        }
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe221 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe221 -p 8082:80 rahulpatel123/2febimg:v1'
                    sshagent(['ssh-keypair']) {
                        //chnage the private ip in below code
                        // sh "docker run -itd --name My-first-containe211 -p 8082:80 rahulpatel123/2febimg:v1"
