pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t rahulpatel123/img33 .'
                    sh 'docker images'
                }
            }
        }
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub_credentials', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push rahulpatel123/img33'
                }
            }
        }
     ...
     stage('Deploy') {
            steps {
               script {
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe33 -p 8081:80 rahulpatel123/img33'
